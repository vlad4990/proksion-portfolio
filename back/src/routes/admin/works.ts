    ...
    .post(
      ...
      ({ body, set }) =>
        guarded(set, () => {
          ...
          // Boolean-флаги показа (чекбоксы админки): boolean снаружи → 0|1 в БД.
          const seamless = optBoolean(b, 'seamless') === true ? 1 : 0
          const carousel = optBoolean(b, 'carousel') === true ? 1 : 0
    ...
    .patch(
      ...
      ({ params, body, set }) =>
        guarded(set, () => {
          ...
          const carousel = optBoolean(b, 'carousel')
          if (carousel !== undefined) patch.carousel = carousel ? 1 : 0
