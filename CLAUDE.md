...
- **Модалка работы** (редизайн 2026-08-19): без карусели — вертикальная лента картинок; десктоп НЕ фулскрин (затемнение + прозрачный диалог, мета справа при 2+ картинках / снизу при одной, красный крестик снаружи, размер от первой картинки: поля `--modal-edge-y/x`), мобайл — фулскрин-шторка снизу с метой под лентой. У работы есть флаг **«единое полотно»** (`work.seamless`, чекбокс в админке): лента рендерится стык-в-стык, без `--tile-gap` — для работ, нарезанных из одного макета. Второй флаг — **«карусель на десктопе»** (`work.carousel`, миграция 0004, 2026-09-01): при 2+ картинках десктопная модалка вместо ленты показывает горизонтальную псевдо-карусель (`WorkCarousel.tsx`: центральный слайд + соседи в ступенчатом градиентном блюре у краёв экрана, стрелки, колесо/трекпад/клик/←→; мобилка флаг игнорирует; блюр — БЕЗ mask-image и backdrop-filter, Chrome рисует их с неверной геометрией). Анимации открытия/закрытия — рукописный FLIP из тайла листинга + WAAPI-спринги (`front/src/lib/spring.ts` + `flip.ts`), **без** motion/framer.
...
- **Миграция `0004_work_carousel.sql`** (2026-09-01) — `work.carousel` (0|1 + CHECK): флаг «карусель на десктопе», горизонтальная псевдо-карусель в десктопной модалке вместо вертикальной ленты (при 2+ картинках; мобилка игнорирует). Сквозная фича по образцу `seamless`: `WorkDetail.carousel: boolean`, чекбокс в `WorkDialog`, `carousel?: boolean` в `WorkInput`/`WorkPatchInput`.
