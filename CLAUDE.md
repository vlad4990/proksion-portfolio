...
- **Модалка работы** (редизайн 2026-08-19): без карусели — вертикальная лента картинок; десктоп НЕ фулскрин (затемнение + прозрачный диалог, мета справа при 2+ картинках / снизу при одной, красный крестик снаружи, размер от первой картинки: поля `--modal-edge-y/x`), мобайл — фулскрин-шторка снизу с метой под лентой. Анимации открытия/закрытия — рукописный FLIP из тайла листинга + WAAPI-спринги (`front/src/lib/spring.ts` + `flip.ts`), **без** motion/framer.
