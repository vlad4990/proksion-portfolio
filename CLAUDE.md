...
- **Модалка работы** (редизайн 2026-08-19): без карусели — вертикальная лента картинок; десктоп НЕ фулскрин (затемнение + прозрачный диалог, мета справа при 2+ картинках / снизу при одной, красный крестик снаружи, размер от первой картинки: поля `--modal-edge-y/x`), мобайл — фулскрин-шторка снизу с метой под лентой. У работы есть флаг **«единое полотно»** (`work.seamless`, чекбокс в админке): лента рендерится стык-в-стык, без `--tile-gap` — для работ, нарезанных из одного макета. Анимации открытия/закрытия — рукописный FLIP из тайла листинга + WAAPI-спринги (`front/src/lib/spring.ts` + `flip.ts`), **без** motion/framer. Фон под модалкой — **background-локация** (2026-09-02): пока открыта работа, фоновые Routes рендерятся по локации листинга-источника (`useBackgroundLocation` в `App.tsx`), а не по URL работы — открытие с `/projects` больше не подменяет фон на страницу подкатегории; `workOpen` фоновые экраны категории получают пропом (useLocation внутри `<Routes location>` видит подменённую локацию).
