...
3. **Секции категорий** (по одной на категорию, порядок = `sort_order`; верхняя
   ...
   - **`showcase`** (KUPIKOD, PRESS F): голова = слева Title Line (номер `01`
     ...
     (14px bold accent → `/projects/:cat`). Витрина: **Row A** высота 486 —
     hero-тайл ~864px (с подписью-пилюлей внизу слева: фон `--bg`, 12px,
     текст = title работы) + колонка из 2 тайлов; **Row B** высота 256 — 4 тайла;
     зазоры `--tile-gap`. Тайлы — обрезка `object-fit: cover` под слот.
   - **`strip`** (РИСОВАНИЕ — 4 тайла h380, SKETCHBOOK — 5 тайлов h230): лёгкая
     голова = Title Line + справа однострочное описание (13px `--fg-dim`,
     из `category.description`); один ряд тайлов фиксированной высоты.
   - **`cards`** (UI/UX КЕЙСЫ): лёгкая голова + ряд из карточек (рамка hairline):
     превью-тайл h340 + текст (паддинг 24): title работы (`--t-title` bold
...
`showcase` = hero-тайл h240 на всю ширину + пары тайлов (`--tile-gap-mob`);
`strip` = пары/тройка тайлов h110; `cards` = карточки в столбик. Футер: CTA
