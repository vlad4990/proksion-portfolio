// ProjectsScreen — full-page projects listing.
//   Top nav, then left rail (collapsible categories) + right masonry grid.
  ...
  return (
    <div style={{ marginBottom: 36 }}>
      <button onClick={onClickGroup} style={{
        ...
        padding: 0,
        cursor: 'pointer',
        fontFamily: 'Stengazeta, sans-serif', fontWeight: 700,
      ...
      onMouseEnter={e => { if (!active) e.currentTarget.style.color = '#e4e4e4'; }}
      onMouseLeave={e => { if (!active) e.currentTarget.style.color = '#bfbfbf'; }}
      >
        {active && (
          <img
...
function ProjectsScreen() {
  const [active, setActive] = React.useState(['pressf', 'vitriny']);
  ...
  // Pinterest-style masonry tiles. Real projects rendered with imagery,
  // others are tonal placeholders that read as future cases.
  const tiles = [
    { h: 320, fill: '#d9d9d9' },
    { h: 240, fill: '#bfbfbf' },
  ...
  return (
    ...
    >
      ...
      {/* Tile grid — masonry via CSS columns */}
      <div style={{
        position: 'absolute',
        left: 675, top: 196,
        width: 1170,
