...
**Фон под модалкой — background-локация** (правка 2026-09-02). Пока открыта работа, фоновые `Routes` рендерятся НЕ по реальному URL, а по локации листинга, с которого её открыли (`useBackgroundLocation` в `App.tsx` + `<Routes location={background}>`); сама модалка — отдельным условным `<Routes>` по реальной локации. Иначе открытие работы с корневой `/projects` (или таба «ВСЕ») подменяло фон за затемнением на страницу подкатегории — весь листинг перерисовывался и дёргал FLIP-анимацию открытия. Deep-link фона в истории не имеет — фоном берётся родительский `/projects/:cat/:sub` из пути работы. Два следствия: (1) `useLocation()` внутри `<Routes location={…}>` возвращает ПОДМЕНЁННУЮ локацию — реальный URL из фоновых экранов не увидеть, поэтому флаг `workOpen` (открыта ли работа поверх) `CategoryScreen`/`MobileCategory` получают пропом из `App`; (2) параметра `:work` в фоновых экранах больше нет — гард заголовка вкладки живёт на том же `workOpen`.
...
Шапка/мини-хедер и футер — общий хром: они **сознательно оставлены как были** и не приводились к макетам (в дизайне у шапки другой вордмарк, красный активный пункт и чип «ОТКРЫТ К ПРОЕКТАМ»). Мобильный статус-бар из фрейма не делаем — та же договорённость, что на `/projects`. Высота шапки — **80px на всех тирах** (`--nav-h` во всех desktop-тирах и `--mob-content-top` = `--mob-status` 24 + `--mob-header-h` 56 на мобайле; правка 2026-09-03 по просьбе заказчика — прежние 120/96/76 и 112 читались слишком крупно).
...
- **Карусели нет** — картинки работы рендерятся вертикальной лентой (`flex-col`, gap `--tile-gap(-mob)`), первая eager, остальные `loading="lazy"`. `?img=` больше не используется (легаси-ссылки его игнорируют).
